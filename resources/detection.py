...
def toImage(output_dict, image_np, key, path_detect):
    ...
    vis_util.visualize_boxes_and_labels_on_image_array(
        ...
        line_thickness=8)
    # 画图输出
    plt.imsave(path_detect + '/' + key + '.png', image_np)
...
def detection():
  algorithm = 'ssd'
  PATH_TO_FROZEN_GRAPH = os.path.abspath(os.path.join(os.path.dirname(__file__), '../train_model/' + algorithm + '/frozen_inference_graph.pb'))
  ...
  imgs_path = glob.glob(os.path.join(TRAIN_DETECTIONS_PATH, '*.png'))
  # 对每个文件进行识别
  for path in imgs_path:
      with tf.gfile.GFile(path, 'rb') as fid:
    # 将图片文件转换成 Image 对象
        image = Image.open(BytesIO(fid.read()))
        key = os.path.split(path)[-1].replace('.png', '')
        # 获取图片识别数据的部分
